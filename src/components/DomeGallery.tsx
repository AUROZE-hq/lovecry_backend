...
const DEFAULT_IMAGES: ImageItem[] = [
  { src: 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=774&auto=format&fit=crop', alt: 'Hope Summit' },
  { src: 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?q=80&w=774&auto=format&fit=crop', alt: 'Community Aid' },
  { src: 'https://images.unsplash.com/photo-1532629345422-7515f3d16bb6?q=80&w=774&auto=format&fit=crop', alt: 'Charity Run' },
  { src: 'https://images.unsplash.com/photo-1509099836639-18ba1795216d?q=80&w=774&auto=format&fit=crop', alt: 'Volunteer Day' },
  { src: 'https://images.unsplash.com/photo-1559027615-cd4628902d4a?q=80&w=774&auto=format&fit=crop', alt: 'Global Help' },
  { src: 'https://images.unsplash.com/photo-1497486751825-1233686d5d80?q=80&w=774&auto=format&fit=crop', alt: 'Education Drive' },
  { src: 'https://images.unsplash.com/photo-1593113598332-cd288d649433?q=80&w=774&auto=format&fit=crop', alt: 'Youth Camp' },
];
...
function buildItems(pool: (ImageItem | string)[], seg: number): TileCoord[] {
  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        ...
        pointerTypeRef.current = pe.pointerType || 'mouse';
        if (pointerTypeRef.current === 'touch') { (event as Event).preventDefault(); lockScroll(); }
        draggingRef.current = true;
        cancelTapRef.current = false;
        movedRef.current = false;
        startRotRef.current = { ...rotationRef.current };
        startPosRef.current = { x: pe.clientX, y: pe.clientY };
        const potential = (pe.target as HTMLElement).closest?.('.item__image') as HTMLElement | null;
      ...
      onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
        ...
        const dxTotal = pe.clientX - startPosRef.current.x;
        const dyTotal = pe.clientY - startPosRef.current.y;
        ...
        if (last) {
          ...
          if (startPosRef.current) {
            const dx = pe.clientX - startPosRef.current.x;
            const dy = pe.clientY - startPosRef.current.y;
            const dist2 = dx * dx + dy * dy;
            const TAP_THRESH_PX = pointerTypeRef.current === 'touch' ? 10 : 6;
            if (dist2 <= TAP_THRESH_PX * TAP_THRESH_PX) isTap = true;
          }

          let [vMagX, vMagY] = velArr as number[];
          const [dirX, dirY] = dirArr as number[];
          let vx = vMagX * dirX;
          let vy = vMagY * dirY;
