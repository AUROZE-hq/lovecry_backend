...
export default function ImpactSummaryBar({ totals }: { totals: ImpactSummaryTotals }) {
  ...
  return (
    <section className="overflow-hidden rounded-3xl bg-[#F3F0F5] px-4 py-8 sm:px-8 sm:py-10 lg:px-10" aria-labelledby="impact-together">
      <div className="flex flex-col items-center gap-6 sm:gap-8 lg:flex-row lg:justify-between">
        <h2
          id="impact-together"
          className="flex items-center justify-center gap-3 text-center font-news-headline text-2xl text-[#2A1A38] sm:text-3xl lg:text-left"
        >
          Our Impact Together
          <Heart className="h-5 w-5 shrink-0 text-[#693492] sm:h-6 sm:w-6" aria-hidden />
        </h2>
        <dl className="grid w-full grid-cols-1 gap-5 min-[400px]:grid-cols-2 sm:grid-cols-2 lg:w-auto lg:flex lg:flex-wrap lg:justify-end lg:gap-8 xl:gap-10">
          {items.map((item, index) => {
            ...
            return (
              <div
                key={item.key}
                className={`min-w-0 text-center ${index > 0 ? 'lg:border-l lg:border-[#D9D0E3] lg:pl-8 xl:pl-10' : ''}`}
              >
                <dt className="flex items-center justify-center gap-2 text-[11px] font-semibold uppercase leading-tight tracking-wider text-[#8A7798] sm:text-xs">
                  <Icon className="h-4 w-4 shrink-0 text-[#A084B7]" aria-hidden />
                  <span className="max-w-[9.5rem]">{item.label}</span>
                </dt>
                <dd className="mt-2 font-news-headline text-2xl text-[#2A1A38] sm:text-3xl">{item.value}</dd>
              </div>
            );
          })}
        </dl>
